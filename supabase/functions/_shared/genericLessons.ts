/** Keep Intriguing Insights generic — never address the viewer's company. */
...
export function sanitizeLessonText(text: string, companyName: string | null | undefined): string {
  if (!companyName?.trim() || !mentionsViewerCompany(text, companyName)) {
    return text;
  }
  return stripViewerCompanyMentions(text, companyName);
}
